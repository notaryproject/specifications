...
1. **Generate signature:** Using notation CLI or any other compliant signing tool, sign the OCI artifact. The signing tool should follow the following guideline.
    ...
    1. Generate signature.
        ...
        1. If the user is signing under signing scheme [`notary.x509`](./signing-scheme.md/#notaryx509) and wants to authenticate the signing time, extract the [cryptographic signature](./signature-specification.md#signature-envelope) (the digital signature computed on payload and signed attributes) from the signature envelope generated in the previous step. Send it to a [RFC-3161](https://datatracker.ietf.org/doc/html/rfc3161.html) compliant timestamp authority (TSA). Validate and embed the received timestamp countersignature to the *Timestamp Signature* unsigned attribute of the signature envelope. Otherwise, continue to the next step.
