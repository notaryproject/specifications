...
- **Timestamp Signature**: An OPTIONAL countersignature. It is generated by a trusted third party to prove that the `Cryptographic signature (the digital signature computed on payload and signed attributes)` was created on or before the time point been stamped. The only supported such trusted third party is [RFC3161][ietf-rfc3161] compliant `Timestamp Authority (TSA)`. When provided, this claim is only validated and used under signing scheme [`notary.x509`](./signing-scheme.md/#notaryx509).
...
An `authentic signing time` is a signing time authenticated by a `Signing Authority (SA)`. It allows a verifier to determine if the signature was generated within the certificate's validity. This is ONLY supported under signing scheme [`notary.x509.signingAuthority`](./signing-scheme.md/#notaryx509signingauthority), the `authentic signing time` is determined and MUST be provided by the *Authentic Signing Time* signed attribute in the signature envelope. Signing scheme [`notary.x509`](./signing-scheme.md/#notaryx509) does NOT have the concept `authentic signing time`.

*Special note*: The *Timestamp Signature* unsigned attribute does NOT provide an `authentic signing time` under signing scheme [`notary.x509`](./signing-scheme.md/#notaryx509). This attribute ONLY proves that the `Cryptographic signature` in the signature envelope was created on or before the time point been stamped. The `time point been stamped` here means the particular time point at which the `Timestamp Authority (TSA)` receives the `Cryptographic signature`.
