...
- **Timestamp Signature**: An OPTIONAL countersignature. It is generated by a trusted third party to prove that the `primitive signature (the digital signature computed on payload and signed attributes)` was generated before the time point been stamped. The only supported trusted third party is [RFC3161][ietf-rfc3161] compliant `Timestamp Authority (TSA)`. When provided, this claim is only validated and used under signing scheme [`notary.x509`](./signing-scheme.md/#notaryx509).
...
> [!NOTE]
> The *Timestamp Signature* unsigned attribute MUST NOT be used as an `authentic signing time` under signing scheme [`notary.x509`](./signing-scheme.md/#notaryx509) as successful verification of the timestamp signature only implies that the `primitive signature` in the signature envelope was generated before the time point been stamped. The phrase `time point been stamped` refers to the specific time point at which the `Timestamp Authority (TSA)` receives the `primitive signature`.
