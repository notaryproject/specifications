...
* `notary.x509` can only use a timestamp countersignature to prove that the signature was generated before the time point been stamped by the `Timestamp Authority (TSA)`.
...
* `notary.x509.signingAuthority` MUST use a date/time attribute that is generated by the signing service as part of the original signature to determine `authentic signing time`.
