...
1. Execute the plugin with `get-plugin-metadata` command
    1. If plugin supports capability `SIGNATURE_GENERATOR.ENVELOPE`
        ...
        2. If the plugin supports TSA Timestamping under signing scheme `notary.x509`, the plugin SHOULD request and include TSA timestamp countersignature in the signature envelope at this step.
