...
1. Execute the plugin with `get-plugin-metadata` command
    1. If plugin supports capability `SIGNATURE_GENERATOR.ENVELOPE`
        ...
        2. If plugin supports timestamping under signing scheme [`notary.x509`](./signing-scheme.md/#notaryx509), the plugin MUST request and include TSA timestamp countersignature in the signature envelope at this step. Plugin MAY also allow users to specify their own TSA server URL via [pluginConfig](#plugin-configuration).
