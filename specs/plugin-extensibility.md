...
1. Execute the plugin with `get-plugin-metadata` command
    1. If plugin supports capability `SIGNATURE_GENERATOR.ENVELOPE`
        ...
        2. If plugin supports TSA Timestamping under signing scheme `notary.x509`, the plugin MUST request and include TSA timestamp countersignature in the signature envelope at this step. Plugins MAY also allow users to specify their own TSA server URL via plugin-config.
