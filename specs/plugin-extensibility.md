...
5. Execute the plugin with `get-plugin-metadata` command
    1. If plugin supports capability `SIGNATURE_GENERATOR.RAW`
        ...
        2. Generate the payload to be signed for [JWS](./signature-specification.md#supported-signature-envelopes) envelope format.
           ...
           2. Create the Notary signing payload as defined [here](./signature-specification.md#payload).
...
* The interface MUST allow processing of [extended attributes](./signature-specification.md#extended-attributes) that are not part of Notary [standard attributes](./signature-specification.md#standard-attributes).
...
This allows Notary implementations to perform the same logic themselves, if they choose to.
...
5. Populate `verify-signature` request. NOTE: The processing order of remaining known attributes does not matter as long as they are processed before the end of signature verification workflow.
    1. Set `request.signature.criticalAttributes` to the set of [standard Notary attributes](./signature-specification.md#standard-attributes) that are marked critical, from the signature envelope.
...
```jsonc
{
  "contractVersion" : "<major-version.minor-version>",
   ...
   "signature" : {
    // Array of all Notary defined critical attributes and their values
