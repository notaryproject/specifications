...
- **[Signature Verification](#signature-verification)**: Describes how signatures are evaluated using the policy, to determine if a signed artifact is authentic. This section is meant for implementors of Notary Project standards.
...
All examples use the actors defined in Notary Project [scenario](https://github.com/notaryproject/notaryproject/blob/main/scenarios.md#scenario-0-build-publish-consume-enforce-policy-deploy)
...
- **`trustPolicies`**(*string-array of objects map*): This REQUIRED property represents a collection of trust policies.
  ...
  An *object* that specifies a predefined verification level, with an option to override Notary Project defined verification level if user wants to specify a [custom verification level](#custom-verification-level).
...
- For a given artifact, if there is no applicable trust policy then Notary Project specification implementation MUST consider the artifact as untrusted and fail signature verification.
...
Notary Project specification implementation MUST support BaseCRLs and Delta CRLs.
Notary Project specification implementation MAY support Indirect CRLs.
Notary Project specification implementation supports only HTTP CRL URLs.
...
For each CDP location, Notary Project specification implementation will try to download the CRL for the default threshold of 10 seconds.
...
Notary Project specification implementation will wait for a default threshold of 5 seconds to receive an OCSP response.
...
**Q: Does Notary Project specification supports `n` out of `m` signatures verification requirement?**

**A:** Notary Project specification doesn't support n out m signature requirement verification scheme.
Signature verification workflow succeeds if verification succeeds for at least one signature.

**Q: Does Notary Project specification support overriding of revocation endpoints to support signature verification in disconnected environments?**
