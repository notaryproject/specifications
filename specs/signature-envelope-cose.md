...
1. Instead of `targetArtifact` as shown under [COSE Payload](./signature-envelope-cose.md/#cose-payload), the payload of a `COSE Hash Envelope` is the hash of the original content bytes that is being signed. The hash algorithm MUST be deduced from the signing certificate's public key (see [Algorithm Selection](./signature-specification.md#algorithm-selection)).
2. New protected header parameters under `COSE Hash Envelope` only:
...
3. Header parameters that MUST NOT be present in `COSE Hash Envelope`:
